...
parser.add_argument("-f", "--file", help="The data file to parse", default="data/morn_route.csv")
...
df = pd.read_csv(clean_file, names=["date", "time", "weekday", "long time", "length"])
...
for weekday in grouped.index.levels[0]:
    labels = list(grouped.index.levels[1])
